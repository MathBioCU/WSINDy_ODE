function [err_wsindy,err_sindy,tp_w,tp_s,t_dd,x_dd,F_dd,lambda_hat_w,lambda_hat_s,resid,resid_true] = display_results(w_sparse,true_nz_weights,w_sparse_sindy,loss_wsindy,...
...
elseif toggle_plot_ddd>2
    ...
    if n==1
        errs = abs(F_dd(vec_grids{1})-rhs(vec_grids{1}));
        plot(vec_grids{1},errs)
    elseif n==2
        [xx,yy] = meshgrid(vec_grids{:});        
        vec_points = [xx(:)';yy(:)'];
        errs = vecnorm(F_dd(vec_points)-rhs(vec_points),2,1);
        errs = reshape(errs,np,np);
        imagesc(vec_grids{:},errs)
        colorbar
    elseif n==3
         errs = vecnorm(F_dd(x')-rhs(x'),2,1);
