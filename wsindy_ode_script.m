...
ode_num = 3;                        % select ODE from list below
tol_ode = 1e-12;                    % set tolerance (abs and rel) of ode45
noise_ratio = 0;                 % set ratio of noise to signal (L2 sense), if negative, sets sigma to -noise_ratio
...
%%% library
polys = 0:6;                        % monomial powers to include in library
...
tau = 25;                           % tau > 1: [tau,tauhat]=[m,p] directly. else...
tauhat = 9;                         % tauhat > 0: tauhat = width-at-half-max. tauhat < 0: use corner point
K_frac = 0.10;                      % K_frac > 1: K (number of test functions) = K_frac. K_frac < 1: K = T*K_frac (T = number of timespoints)
overlap_frac = 1;                   % fraction of allowable support overlap between neighboring test functions 
relax_AG = 0.1;                       % convex combination between uniform and gradient-adapted placement of test functions

%%% optimization
scale_Theta = 0;                    % toggle normalize columns of Theta_0 by norm = scale_Theta_0
useGLS = 10^-0;                     % useGLS = 0 calls OLS. useGLS > 0 calls GLS with cov (1-useGLS)*C + useGLS*diag(C), C = Vp*Vp'
lambda = 10.^(linspace(-4,0,50));   % lambda values for MSTLS
gamma = 10.^-inf*sqrt(noise_ratio); % Tikhonov regularization
alpha_loss = 0.5;                   % convex combination between rel. resid. and rel. sparsity used in lambda loss function
...
toggle_print_w = 0;                 % print weights
toggle_plot = 0;                    % print data, distribution of test functions, covariance sparsity
toggle_plot_ddd = 0;                % toggle simulate data-driven dynamics (ddd), 1 calls WSINDy weights, 2 calls SINDy weights, >2 displays error in vector field on a plot with toggle_plot_ddd points in each dimensions (dims 1,2,3) 
...
toggle_plot_derivs = 0;             % plot weak derivatives is using adaptive grid
...
[err_wsindy,err_sindy,tp_w,tp_s,t_dd,x_dd,F_dd,lambda_hat_w,lambda_hat_s,resid,resid_true] = display_results(w_sparse,true_nz_weights,w_sparse_sindy,loss_wsindy,...
    ...
    toggle_plot_loss,toggle_plot_derivs,toggle_plot_approx_sys,bweaks,useFD,dxobs_0,...
    tol_ode,x0,ode_name,ode_params,tags);